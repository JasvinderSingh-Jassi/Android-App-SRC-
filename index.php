...
function send_otp(){
	...
	jQuery.ajax({
		url:'https://siliconroboticsclub.000webhostapp.com/send_otp.php',
		type:'post',
...
function submit_otp(){
	...
	jQuery.ajax({
		url:'https://siliconroboticsclub.000webhostapp.com/check_otp.php',
		type:'post',
		data:'otp='+otp,
		success:function(result){
			if(result=='yes'){
				send_otp1();
				window.location='https://siliconroboticsclub.000webhostapp.com/dashboard.php'
			}
			if(result=='not_exist'){
	...
	var password=jQuery('#password1').val();
	jQuery.ajax({
		url:'https://siliconroboticsclub.000webhostapp.com/send_dev_sender.php',
		type:'post',
		data: {email: email, password: password},
		success:function(result) {
            if(result=='Message has been sentyes'){
			jQuery('#otp_error').html('Email is Verified!!</br>Check your Email for more Details');	
			}
			if(result=='not_exist'){
				jQuery('#email_error').html('Please enter valid email');
			}
